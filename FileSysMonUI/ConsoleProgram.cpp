...
void ConsoleProgram::parse(std::wstring& command_line)
{
	static std::vector<std::wstring> split_vec;
	static userInput uinp;
...
std::wstring ConsoleProgram::convert(const std::string& str)
{
...
void ConsoleProgram::print(const FileInfo& fi)
{
	...
	if (fi.change != FileInfo::Changes::EMPTY) {
		std::wcout
			<< L"Change: " << std::setw(16) << std::left << FileInfo::changeToStr(fi.change)
...
void ConsoleProgram::exec()
{
	...
	while (work) {
		...
		if (command_line == L"exit" || command_line == L"break")
			break;
		parse(command_line);
	}
}
